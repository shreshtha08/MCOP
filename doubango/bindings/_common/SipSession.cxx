...
//MCPTT MBMS
bool SipSession::startMBMSManager(bool isStart){
	tsk_bool_t isStartManager;
	if(isStart){
		isStartManager=tsk_true;
	}else{
		isStartManager=tsk_false;
	}

	return (tsip_ssession_set(m_pHandle,
		TSIP_SSESSION_MBMS_START_MANAGER(isStartManager),
...
{
	if(m_pHandle==NULL || m_pHandle==0 || !m_pHandle)return 0;
...
bool CallSession::callGroup(char** user_list, int user_count, twrap_media_type_t media, ActionConfig* config)
{
	...
	tsip_ssession_set(m_pHandle,
		TSIP_SSESSION_SET_PTT_GROUP_MEMBERS(user_list, user_count),
		TSIP_SSESSION_SET_NULL());
...
bool CallSession::setPoCQoE(tmedia_poc_qoe_profile_t profile, tmedia_poc_qoe_profile_strength_t strength)
{
	return (tsip_ssession_set(m_pHandle,
			 TSIP_SSESSION_SET_MEDIA(
				TSIP_MSESSION_SET_POC_QOE(profile, strength),
				TSIP_MSESSION_SET_NULL()
			),
			TSIP_SSESSION_SET_NULL()) == 0);
}
...
bool CallSession::setMcpttCallback(const McpttCallback* pMcpttCallback)
{
	...
	const MediaSessionMgr* pMgr;
	if((pMgr = getMediaMgr()) && (pWrappedMgr = pMgr->getWrappedMgr())){
		if((m_pMcpttCallback = pMcpttCallback)){
			return (tmedia_session_mgr_set_mcptt_cbfn((tmedia_session_mgr_t*)pWrappedMgr, this, twrap_mcptt_cb) == 0);
		}
	}
	return false;
}

bool CallSession::requestMcpttToken(ActionConfig* config/*=tsk_null*/ )
{
	const tmedia_session_mgr_t* pWrappedMgr;
	const MediaSessionMgr* pMgr;
	if((pMgr = getMediaMgr()) && (pWrappedMgr = pMgr->getWrappedMgr())){
		return (tmedia_session_mgr_request_mcptt_token((tmedia_session_mgr_t*)pWrappedMgr) == 0);		
	}
	TSK_DEBUG_ERROR("No media manager");
	return false;
}

bool CallSession::releaseMcpttToken(ActionConfig* config/*=tsk_null*/ )
{
	const tmedia_session_mgr_t* pWrappedMgr;
	const MediaSessionMgr* pMgr;
	if((pMgr = getMediaMgr()) && (pWrappedMgr = pMgr->getWrappedMgr())){
		return (tmedia_session_mgr_release_mcptt_token((tmedia_session_mgr_t*)pWrappedMgr) == 0);		
	}
	TSK_DEBUG_ERROR("No media manager");
	return false;
}
...
bool CallSession::startMbmsManager(ActionConfig* config/*=tsk_null*/,const char* remoteIP,int remotePort, const char* localIface, int localIfaceIdx)
{
	...
	const MediaSessionMgr* pMgr;
	if((pMgr = getMediaMgr()) && (pWrappedMgr = pMgr->getWrappedMgr())){
		return (tmedia_session_mgr_start_mbms_manager((tmedia_session_mgr_t*)pWrappedMgr,remoteIP,remotePort, localIface, localIfaceIdx) == 0);		
	}
	TSK_DEBUG_ERROR("No media manager");
	return false;
}
...
bool CallSession::stopMbmsManager(ActionConfig* config/*=tsk_null*/ )
{
	...
	const MediaSessionMgr* pMgr;
	if((pMgr = getMediaMgr()) && (pWrappedMgr = pMgr->getWrappedMgr())){
		return (tmedia_session_mgr_stop_mbms_manager((tmedia_session_mgr_t*)pWrappedMgr) == 0);		
	}
	TSK_DEBUG_ERROR("No media manager");
	return false;
}
bool CallSession::startMbmsMedia(ActionConfig* config/*=tsk_null*/, const char* mediaIP, int mediaPort, int mediaCtrlPort)
{
	const tmedia_session_mgr_t* pWrappedMgr;
	const MediaSessionMgr* pMgr;
	if((pMgr = getMediaMgr()) && (pWrappedMgr = pMgr->getWrappedMgr())){
		return (tmedia_session_mgr_start_mbms_media((tmedia_session_mgr_t*)pWrappedMgr, mediaIP, mediaPort, mediaCtrlPort) == 0);		
	}
	TSK_DEBUG_ERROR("No media manager");
	return false;
}
bool CallSession::setMcpttMbmsCallback(const McpttMbmsCallback* pMcpttMbmsCallback)
{
	const tmedia_session_mgr_t* pWrappedMgr;
	const MediaSessionMgr* pMgr;
	if((pMgr = getMediaMgr()) && (pWrappedMgr = pMgr->getWrappedMgr())){
		if((m_pMcpttMbmsCallback = pMcpttMbmsCallback)){
			return (tmedia_session_mgr_set_mcptt_mbms_cbfn((tmedia_session_mgr_t*)pWrappedMgr, this, twrap_mcptt_mbms_cb) == 0);
		}
	}
	return false;
}
...
const char* CallSession::getPTTMcpttGroupIdentity()
{
	...
	//return tsip_ssession_get_party_uri(m_pHandle);
}

int CallSession::getPTTMcpttGroupMembers()
{
	return tsip_ssession_get_ptt_mcptt_group_members(m_pHandle);

}

const char* CallSession::getPTTMcpttGroupMemberAtPosition(int pos)
{
	return tsip_ssession_get_ptt_mcptt_group_member_at_position(m_pHandle, pos);
}
//
char* CallSession::getSipPartyUri()
{
...
const XcapCallback* CallSession::getXcapCallback() const
{
	return m_pPTTXcapCallback;
}
const McpttCallback* CallSession::getMcpttCallback() const
{
	return m_pMcpttCallback;
}
const McpttMbmsCallback* CallSession::getMcpttMbmsCallback() const
{
	return m_pMcpttMbmsCallback;
}
...
McpttSession::McpttSession(SipStack* pStack, McpttCallback* pCallback)
: InviteSession(pStack), m_pCallback(pCallback)
{
	tsip_ssession_set(m_pHandle,
		TSIP_SSESSION_SET_MEDIA(
			TSIP_MSESSION_SET_MCPTT_CB(twrap_mcptt_cb),
			TSIP_MSESSION_SET_NULL()
		),
		TSIP_SSESSION_SET_NULL());
}

McpttSession::McpttSession(SipStack* pStack, tsip_ssession_handle_t* pHandle)
: InviteSession(pStack, pHandle)
{
	tsip_ssession_set(m_pHandle,
		TSIP_SSESSION_SET_MEDIA(
			TSIP_MSESSION_SET_MCPTT_CB(twrap_mcptt_cb),
...
bool McpttSession::setCallback(McpttCallback* pCallback)
{
	m_pCallback = pCallback;
	return true;
}

bool McpttSession::requestToken(ActionConfig* config/*=tsk_null*/)
{
	return true;
}
...
MessagingLocationSession::MessagingLocationSession(SipStack* pStack, tsip_ssession_handle_t* pHandle)
: SipSession(pStack, pHandle)
{
}

MessagingLocationSession::~MessagingLocationSession()
{
}


bool MessagingLocationSession::send(const void* payload, unsigned len, ActionConfig* config/*=tsk_null*/)
{
	int ret;
	const tsip_action_handle_t* action_cfg = config ? config->getHandle() : tsk_null;
	if(payload && len){
		ret = tsip_api_message_send_message_location(m_pHandle,
			TSIP_ACTION_SET_PAYLOAD(payload, len),
			TSIP_ACTION_SET_CONFIG(action_cfg),
			TSIP_ACTION_SET_NULL());
	}
...
bool MessagingLocationSession::accept(ActionConfig* config/*=tsk_null*/)
{
	const tsip_action_handle_t* action_cfg = config ? config->getHandle() : tsk_null;
	return (tsip_api_common_accept(m_pHandle,
		TSIP_ACTION_SET_CONFIG(action_cfg),
...
bool MessagingLocationSession::reject(ActionConfig* config/*=tsk_null*/)
{
	const tsip_action_handle_t* action_cfg = config ? config->getHandle() : tsk_null;
	return (tsip_api_common_reject(m_pHandle,
		TSIP_ACTION_SET_CONFIG(action_cfg),
...
MessagingAffiliationSession::MessagingAffiliationSession(SipStack* pStack)
: SipSession(pStack)
{
}

MessagingAffiliationSession::MessagingAffiliationSession(SipStack* pStack, tsip_ssession_handle_t* pHandle)
: SipSession(pStack, pHandle)
{
}

MessagingAffiliationSession::~MessagingAffiliationSession()
{
}


bool MessagingAffiliationSession::send(const void* payload, unsigned len, ActionConfig* config/*=tsk_null*/)
{
...
bool MessagingAffiliationSession::accept(ActionConfig* config/*=tsk_null*/)
{
	const tsip_action_handle_t* action_cfg = config ? config->getHandle() : tsk_null;
	return (tsip_api_common_accept(m_pHandle,
		TSIP_ACTION_SET_CONFIG(action_cfg),
...
bool MessagingAffiliationSession::reject(ActionConfig* config/*=tsk_null*/)
{
	const tsip_action_handle_t* action_cfg = config ? config->getHandle() : tsk_null;
	return (tsip_api_common_reject(m_pHandle,
		TSIP_ACTION_SET_CONFIG(action_cfg),
...
MessagingMbmsSession::MessagingMbmsSession(SipStack* pStack)
: SipSession(pStack)
{
}

MessagingMbmsSession::MessagingMbmsSession(SipStack* pStack, tsip_ssession_handle_t* pHandle)
: SipSession(pStack, pHandle)
{
}

MessagingMbmsSession::~MessagingMbmsSession()
{
}


bool MessagingMbmsSession::send(const void* payload, unsigned len, ActionConfig* config/*=tsk_null*/)
{
	int ret;
	const tsip_action_handle_t* action_cfg = config ? config->getHandle() : tsk_null;
	if(payload && len){
		ret = tsip_api_message_send_message_mbms(m_pHandle,
			TSIP_ACTION_SET_PAYLOAD(payload, len),
			TSIP_ACTION_SET_CONFIG(action_cfg),
			TSIP_ACTION_SET_NULL());
	}
...
bool MessagingMbmsSession::accept(ActionConfig* config/*=tsk_null*/)
{
	const tsip_action_handle_t* action_cfg = config ? config->getHandle() : tsk_null;
	return (tsip_api_common_accept(m_pHandle,
		TSIP_ACTION_SET_CONFIG(action_cfg),
...
bool MessagingMbmsSession::reject(ActionConfig* config/*=tsk_null*/)
{
	const tsip_action_handle_t* action_cfg = config ? config->getHandle() : tsk_null;
	return (tsip_api_common_reject(m_pHandle,
		TSIP_ACTION_SET_CONFIG(action_cfg),
...
PublicationAffiliationSession::PublicationAffiliationSession(SipStack* Stack)
: SipSession(Stack)
{
}

PublicationAffiliationSession::PublicationAffiliationSession(SipStack* pStack, tsip_ssession_handle_t* pHandle)
: SipSession(pStack, pHandle)
{

}

PublicationAffiliationSession::~PublicationAffiliationSession()
{
}

bool PublicationAffiliationSession::publish(const void* payload, unsigned len, ActionConfig* config/*=tsk_null*/)
{
	int ret;
	const tsip_action_handle_t* action_cfg = config ? config->getHandle() : tsk_null;
	if(payload && len){
...
bool PublicationAffiliationSession::unPublish(const void* payload, unsigned len, ActionConfig* config/*=tsk_null*/)
{
	int ret;
	const tsip_action_handle_t* action_cfg = config ? config->getHandle() : tsk_null;
	if(payload && len){
...
/*
bool PublicationAffiliationSession::unPublish(ActionConfig* config)
{
	const tsip_action_handle_t* action_cfg = config ? config->getHandle() : tsk_null;
	return (tsip_api_publish_send_unpublish(m_pHandle,
		TSIP_ACTION_SET_CONFIG(action_cfg),
...
bool RegistrationSession::registerWithMcpttInfo(const void* payload, unsigned len,ActionConfig* config/*=tsk_null*/)
{
	int ret;
	const tsip_action_handle_t* action_cfg = config ? config->getHandle() : tsk_null;
	if(payload && len){
		ret = tsip_api_register_send_register(m_pHandle,
			TSIP_ACTION_SET_PAYLOAD(payload, len),
			TSIP_ACTION_SET_CONFIG(action_cfg),
			TSIP_ACTION_SET_NULL());
	}
	else if(!payload){
		TSK_DEBUG_ERROR("Invalid parameter payload");
	}else if(!len){
		TSK_DEBUG_ERROR("Invalid parameter length");
	}
	return (ret == 0);
}
...
bool SubscriptionSession::subscribe()
{
	return (tsip_api_subscribe_send_subscribe(m_pHandle,
		TSIP_ACTION_SET_NULL()) == 0); /*(tsip_api_subscribe_send_subscribe(m_pHandle,
		TSIP_ACTION_SET_NULL()) == 0);*/
}
...
SubscriptionAffiliationSession::SubscriptionAffiliationSession(SipStack* pStack)
: SipSession(pStack)
{
}

SubscriptionAffiliationSession::SubscriptionAffiliationSession(SipStack* pStack, tsip_ssession_handle_t* pHandle)
: SipSession(pStack, pHandle)
{
...
SubscriptionAffiliationSession::~SubscriptionAffiliationSession()
{
}
...
bool SubscriptionAffiliationSession::unSubscribeAffiliation()
{
