...
int tdav_codec_amrwb_open(tmedia_codec_t* self)
{
	tdav_codec_amr_t* amrwb = (tdav_codec_amr_t*)self;

	if(!TDAV_CODEC_AMR(amrwb)->encoder){
		/* MCPTT client. Does not compile. */
		if(!(TDAV_CODEC_AMR(amrwb)->encoder = E_IF_init())){
			TSK_DEBUG_ERROR("Failed to initialize AMR-WB encoder");
			return -2;
		}
	}

	if(!TDAV_CODEC_AMR(amrwb)->decoder){
		/* MCPTT client. Does not compile. */
		if(!(TDAV_CODEC_AMR(amrwb)->decoder = D_IF_init())){
			TSK_DEBUG_ERROR("Failed to initialize AMR-WB decoder");
			return -2;
		}
...
int tdav_codec_amrwb_close(tmedia_codec_t* self)
{
	tdav_codec_amr_t* amrwb = (tdav_codec_amr_t*)self;

	if(TDAV_CODEC_AMR(amrwb)->encoder){
		/* MCPTT client. Does not compile. */
		E_IF_exit(TDAV_CODEC_AMR(amrwb)->encoder);
		
		TDAV_CODEC_AMR(amrwb)->encoder = tsk_null;
	}

	if(TDAV_CODEC_AMR(amrwb)->decoder){
		/* MCPTT client. Does not compile. */
		D_IF_exit(TDAV_CODEC_AMR(amrwb)->decoder);
		
		TDAV_CODEC_AMR(amrwb)->decoder = tsk_null;
	}

	return 0;
}

tsk_size_t tdav_codec_amrwb_encode(tmedia_codec_t* self, const void* in_data, tsk_size_t in_size, void** out_data, tsk_size_t* out_max_size)
{	
	tdav_codec_amr_t* amr = (tdav_codec_amr_t*)self;
...
char* tdav_codec_amrwb_sdp_att_get(const tmedia_codec_t* codec, const char* att_name)
{
	const tdav_codec_amr_t* amr = (const tdav_codec_amr_t*)codec;
...
tsk_bool_t tdav_codec_amrwb_sdp_att_match(const tmedia_codec_t* codec, const char* att_name, const char* att_value)
{	
	tdav_codec_amr_t* amr;
	if(!(amr = (tdav_codec_amr_t*)codec) || !att_name){
		TSK_DEBUG_ERROR("Invalid parameter");
...
//
//	AMR-WB OA Plugin definition
//

/* constructor */
static tsk_object_t* tdav_codec_amrwb_oa_ctor(tsk_object_t * self, va_list * app)
{
	tdav_codec_amr_t *amrwb_oa = self;
	if(amrwb_oa){
		/* init base: called by tmedia_codec_create() */
		/* init self */
		tdav_codec_amr_init(TDAV_CODEC_AMR(amrwb_oa), tdav_codec_amr_type_wb, tdav_codec_amr_mode_oa);
	}
	return self;
}
/* destructor */
static tsk_object_t* tdav_codec_amrwb_oa_dtor(tsk_object_t * self)
{ 
	tdav_codec_amr_t *amrwb_oa = self;
	if(amrwb_oa){
		/* deinit base */
		tmedia_codec_audio_deinit(amrwb_oa);
		/* deinit self */
		tdav_codec_amr_deinit(TDAV_CODEC_AMR(amrwb_oa));
	}

	return self;
}
/* object definition */
static const tsk_object_def_t tdav_codec_amrwb_oa_def_s = 
{
	sizeof(tdav_codec_amr_t),
	tdav_codec_amrwb_oa_ctor, 
	tdav_codec_amrwb_oa_dtor,
	tmedia_codec_cmp, 
};
/* plugin definition*/
static const tmedia_codec_plugin_def_t tdav_codec_amrwb_oa_plugin_def_s = 
{
	&tdav_codec_amrwb_oa_def_s,

	tmedia_audio,
	tmedia_codec_id_amr_wb_oa,
	"AMR-WB",
	"AMR Wide Band - Octet Aligned (libopencore-amr, libvo-amrwbenc)",
	TMEDIA_CODEC_FORMAT_AMR_WB_OA,
	tsk_true,
	16000, // rate
	...
	tsk_null, // set()
	tdav_codec_amrwb_open,
	tdav_codec_amrwb_close,
	tdav_codec_amrwb_encode,
	tdav_codec_amrwb_decode,
	tdav_codec_amrwb_sdp_att_match,
	tdav_codec_amrwb_sdp_att_get
};
const tmedia_codec_plugin_def_t *tdav_codec_amrwb_oa_plugin_def_t = &tdav_codec_amrwb_oa_plugin_def_s;

//
//	AMR-WB BE Plugin definition
//

/* constructor */
static tsk_object_t* tdav_codec_amrwb_be_ctor(tsk_object_t * self, va_list * app)
{
	tdav_codec_amr_t *amrwb_be = self;
	if(amrwb_be){
		/* init base: called by tmedia_codec_create() */
		/* init self */
		tdav_codec_amr_init(TDAV_CODEC_AMR(amrwb_be), tdav_codec_amr_type_wb, tdav_codec_amr_mode_be);
	}
	return self;
}
/* destructor */
static tsk_object_t* tdav_codec_amrwb_be_dtor(tsk_object_t * self)
{ 
	tdav_codec_amr_t *amrwb_be = self;
	if(amrwb_be){
		/* deinit base */
		tmedia_codec_audio_deinit(amrwb_be);
		/* deinit self */
		tdav_codec_amr_deinit(TDAV_CODEC_AMR(amrwb_be));
	}

	return self;
}
/* object definition */
static const tsk_object_def_t tdav_codec_amrwb_be_def_s = 
{
	sizeof(tdav_codec_amr_t),
	tdav_codec_amrwb_be_ctor, 
	tdav_codec_amrwb_be_dtor,
	tmedia_codec_cmp, 
};
/* plugin definition*/
static const tmedia_codec_plugin_def_t tdav_codec_amrwb_be_plugin_def_s = 
{
	&tdav_codec_amrwb_be_def_s,

	tmedia_audio,
	tmedia_codec_id_amr_wb_be,
	"AMR-WB",
	"AMR Wide Band - Bandwidth-Efficient (libopencore-amr, libvo-amrwbenc)",
	TMEDIA_CODEC_FORMAT_AMR_WB_BE,
	tsk_true,
	16000, // rate
...
const tmedia_codec_plugin_def_t *tdav_codec_amrwb_be_plugin_def_t = &tdav_codec_amrwb_be_plugin_def_s;
...
{	
	...
	if (amr->type == tdav_codec_amr_type_nb) {
		/* Encode */
	...
	} else if (amr->type == tdav_codec_amr_type_wb) {
		/* Encode AMR-WB */
		/* MCPTT client. Does not compile. */
		if((ret_size = E_IF_encode(amr->encoder, amr->encoder_mode, in_data, outbuf, 0)) <= 0){
			TSK_DEBUG_ERROR("E_IF_encode() failed");
			goto bail;
		}
	...
	for(i = 0; (i<toc_entries && (in_size < (in_size*8))) ; i++){
		...
		switch(TDAV_CODEC_AMR(amr)->type){
			...
			case tdav_codec_amr_type_wb:
				...
				if((speech_data = calloc((size + 2/* ToC + '\0' */), sizeof(uint8_t)))){
			...
			if (amr->type == tdav_codec_amr_type_nb) {
			  ...
			  /* MCPTT client. Does not compile. */
			  D_IF_decode(amr->decoder, speech_data, &((short*)*out_data)[out_size/sizeof(short)], 0);
...
static uint8_t tdav_codec_amr_bitbuffer_read(const void* bits, tsk_size_t size, tsk_size_t start, tsk_size_t count)
{
	...
	pad =(uint8_t)( start ? (8 - (start % 8)) : count);
