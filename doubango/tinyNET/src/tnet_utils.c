...
#undef FREE
	...
	*/
	...
	for (i = 0; i < ifc.ifc_len; i+=sizeof(struct ifreq)) {
		sin = (struct sockaddr_in *)&(ifr->ifr_addr);
		TSK_DEBUG_ERROR("Interface %s found", ifr->ifr_name);
		// TODO: IPAddress if needed
		if(/*ioctl(fd, SIOCGIFFLAGS, &ifr) == 0*/1){
			if (!(ifr->ifr_flags & IFF_LOOPBACK) && (ifr->ifr_flags & IFF_UP)){
				if(/*ioctl(fd, SIOCGIFHWADDR, &ifr) == 0*/1){
...
//EHU
tnet_address_t* tnet_get_interface_addr(const char* iface_name, tnet_family_t family)
{
	tsk_bool_t found = tsk_false;
	tnet_interfaces_L_t* interfaces = tsk_null;
	tnet_addresses_L_t* addresses = tsk_null;
	const tnet_interface_t* iface = tsk_null;
	tnet_address_t* address = tsk_null;
	const tsk_list_item_t *item = tsk_null;

	if (iface_name == tsk_null) {
		TSK_DEBUG_ERROR("Incorrect interface name");
		return tsk_null;
	}

	interfaces = tnet_get_interfaces();
	if (interfaces) {
		tsk_list_foreach(item, interfaces) {
			if (!(iface = item->data)) {
				continue;
			}

			TSK_DEBUG_ERROR("Local interface name=%s", iface->description);
			if (tsk_strcmp(iface->description, iface_name) == 0) { //In Linux description == name (it will not work with names in windows)
				found = tsk_true;
				break;
			}
		}
	} else
		TSK_DEBUG_ERROR("No interface information was found");

	if (iface && found == tsk_true) {
		addresses = tnet_get_addresses(family, 1, 0, 0, 0, iface->index); //Unicast addresses only??
		if (addresses) {
			tsk_list_foreach(item, addresses) {
				...
				if (address->ip != tsk_null) 
					break;
			}
		}
	} else
		TSK_DEBUG_ERROR("Interface %s was not found in the machine", iface_name);
	...
	if(addresses){
		tsk_list_foreach(item, addresses){
			...
			if (address && address->ip){
				TSK_DEBUG_INFO("IP Address is %s",address->ip);
			}
		}
	}
	return addresses;
...
int tnet_sockaddrinfo_init(const char *host, tnet_port_t port, enum tnet_socket_type_e type, struct sockaddr_storage *ai_addr, int *ai_family, int *ai_socktype, int *ai_protocol)
{
	...
	for (ptr = result; ptr; ptr = ptr->ai_next){
		...
		*ai = tsk_calloc(1, sizeof (struct addrinfo));
		...
		(*ai)->ai_addr = calloc(1, ptr->ai_addrlen);
...
static tsk_object_t* tnet_interface_ctor(tsk_object_t * self, va_list * app)
{
	...
	if (iface){
		...
		#if HAVE_CRT //Debug memory
		if ((iface->mac_address = (uint8_t*)calloc(mac_address_length, sizeof(uint8_t)))){
