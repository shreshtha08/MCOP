...
int tsip_dialog_request_configure_mcptt(tsip_dialog_t *self){
	if(TSK_LIST_IS_EMPTY(self->record_routes)){
		...
		//MCPTT. Location menssage
			self->uri_remote_target = tsk_object_ref((void*)TSIP_DIALOG_GET_STACK(self)->sessionLocation.p_asserted_identity_location);
		...
		default:
			{
				...
				//Insert Contact in publish and message
				expire=TSK_TIME_MS_2_S(self->expires);
				if(request->line.request.request_type == tsip_OPTIONS || 
					
					request->line.request.request_type == tsip_REGISTER){
					/**** with expires */
					tsk_sprintf(&contact, "m: <%s:%s@%s:%d>;expires=%" PRId64"\r\n", 
						"sip", 
						from_uri->user_name,
						"127.0.0.1", 
						5060,
					...
					/**** without expires */
					if(request->line.request.request_type == tsip_SUBSCRIBE || request->line.request.request_type == tsip_PUBLISH){
		...
		switch(request->line.request.request_type){
			...
			default:
				...
				if(self->uri_local && TSIP_RESPONSE_CODE(response) >= 101 && TSIP_RESPONSE_CODE(response) <= 299){
					...
					TSK_DEBUG_ERROR("No insert contact.");
				}
				break;
		}
	...
	*/
	...
	for(i =0; (WWW_Authenticate = (const tsip_header_WWW_Authenticate_t*)tsip_message_get_headerAt(response, tsip_htype_WWW_Authenticate, i)); i++){
		...
		tsk_list_foreach(item, self->challenges){
			...
			if(tsk_striequals(challenge->realm, WWW_Authenticate->realm) && (numAttemps < numMaxAttemps || WWW_Authenticate->stale || acceptNewVector)){
				/*== (B) ==*/
...
tsk_bool_t tsip_dialog_send_ptt_refer(tsip_dialog_t *self, tsk_list_t* callee_list, tsk_bool_t is_crisis)
{
	...
	content = tsip_dialog_create_resource_list_group(self, callee_list);
	...
	if((refer = tsip_dialog_request_new(TSIP_DIALOG(self), "REFER"))){
		...
		//	tsip_dialog_apply_action(refer, TSIP_DIALOG(self)->curr_action);
		//}
		tsk_istr_t cid;
		char* cid_str = tsk_null;
		char* domain = tsk_null;
		char* refer_to_value = tsk_null;
		char* content_id_value = tsk_null;
		tsk_strrandom(&cid);
		
		domain = tsk_strdup(self->uri_remote->host);
		#if HAVE_CRT //Debug memory
		cid_str = (char*)calloc(tsk_strlen(cid) + tsk_strlen(domain) + 2, sizeof(char));
	#else
		cid_str = (char*)tsk_calloc(tsk_strlen(cid) + tsk_strlen(domain) + 2, sizeof(char));
	#endif //HAVE_CRT
		
		tsk_sprintf(&cid_str, "%s@%s", cid, domain);

		if(is_crisis)
		{
			#if HAVE_CRT //Debug memory
			refer_to_value = (char*)calloc(tsk_strlen(cid_str) + tsk_strlen("Priv-Answer-Mode='Auto'") + 8, sizeof(char));
		
	#else
			refer_to_value = (char*)tsk_calloc(tsk_strlen(cid_str) + tsk_strlen("Priv-Answer-Mode='Auto'") + 8, sizeof(char));
		
	#endif //HAVE_CRT
			tsk_sprintf(&refer_to_value, "<cid:%s?Priv-Answer-Mode='Auto'>", cid_str);
		}
		else
		{
			#if HAVE_CRT //Debug memory
			refer_to_value = (char*)calloc(tsk_strlen(cid_str) + 7, sizeof(char));
		
	#else
			refer_to_value = (char*)tsk_calloc(tsk_strlen(cid_str) + 7, sizeof(char));
		
	#endif //HAVE_CRT
			tsk_sprintf(&refer_to_value, "<cid:%s>", cid_str);
		}
		#if HAVE_CRT //Debug memory
		content_id_value = (char*)calloc(tsk_strlen(cid_str) + 3, sizeof(char));
		
	#else
		content_id_value = (char*)tsk_calloc(tsk_strlen(cid_str) + 3, sizeof(char));
		
	#endif //HAVE_CRT
		tsk_sprintf(&content_id_value, "<%s>", cid_str);

		TSIP_HEADER_ADD_PARAM(refer->Contact, "+g.poc.talkburst", tsk_null);
		if(is_crisis)
			TSIP_HEADER_ADD_PARAM(refer->Contact, "+g.poc.multicast", tsk_null);

		tsip_message_add_headers(refer,
			TSIP_HEADER_DUMMY_VA_ARGS("Refer-To", refer_to_value),
			TSIP_HEADER_REFER_SUB_VA_ARGS(tsk_false),
			TSIP_HEADER_DUMMY_VA_ARGS("Content-Disposition", "recipient-list"),
			TSIP_HEADER_DUMMY_VA_ARGS("Content-ID", content_id_value),
			TSIP_HEADER_DUMMY_VA_ARGS("Require", "multiple-refer, norefersub"),
			tsk_null);
		ret = tsip_message_add_content(refer, "application/resource-lists+xml", TSK_BUFFER_DATA(content), TSK_BUFFER_SIZE(content));
		ret = tsip_dialog_request_send(TSIP_DIALOG(self), refer);
		TSK_OBJECT_SAFE_FREE(refer);

		TSK_FREE(cid_str);
		TSK_FREE(domain);
		TSK_FREE(refer_to_value);
		TSK_FREE(content_id_value);
	}
...
tsk_buffer_t* tsip_dialog_create_resource_list_group(const tsip_dialog_t* self, tsk_list_t* group_members)
{
	tsip_ssession_t* ptsip_ssession_t;
	const tsk_list_item_t* item = tsk_null;
	int count = 0;
...
/*
	...
	tsk_list_foreach(item, group_members){
		tsk_string_t* memberUri = (tsk_string_t*)item->data;
		if(memberUri != tsk_null && memberUri->value != tsk_null)
		{
