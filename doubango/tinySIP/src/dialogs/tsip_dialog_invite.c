...
#include "tinysip/headers/tsip_header_Priv_Answer_Mode.h"
#include "tinysip/headers/tsip_header.h"
#include "tinysip/authentication/tsip_challenge.h"
#include "tinysip/authentication/tsip_bootstrapping.h"
...
static tsk_buffer_t* tsip_dialog_invite_create_resource_list(const tsip_dialog_invite_t* self);
static tsk_buffer_t* tsip_dialog_invite_create_mcpttinfo(const tsip_dialog_invite_t* self);
			...
			if(msg){
				if(TSIP_MESSAGE_IS_RESPONSE(msg)){ /* Response */
					...
					if(TSIP_REQUEST_IS_INVITE(msg)){ // INVITE
						...
						TSK_DEBUG_WARN("Receive notify 001.");
...
{//receive 200OK and Invite Destino. 
	...
	if(!self->msession_mgr){//if the message is UPDATE
		...
		self->msession_mgr = tmedia_session_mgr_create(TSIP_DIALOG_GET_SS(self)->media.type, 
	...
	if (rINFO){
		ret = send_RESPONSE(self, rINFO, 200, "Ok", tsk_false);
		{
			...
			if (self->msession_mgr && TSIP_MESSAGE_HAS_CONTENT(rINFO)){
				...
				else if (tsk_striequals("application/sdp", TSIP_MESSAGE_CONTENT_TYPE(rINFO)))
				{
					//Process multicast INFO message 
					const char* content_ptr = (const char*)TSIP_MESSAGE_CONTENT_DATA(rINFO);
					tsk_size_t content_size = (tsk_size_t)TSIP_MESSAGE_CONTENT_DATA_LENGTH(rINFO);

					if((ret = tsip_dialog_invite_process_ro(self, rINFO))){
						return ret;
					}
					else{
						tsdp_message_t* sdp = tsk_null;
						const tsdp_header_M_t* multicast_media = tsk_null;
						
						const tsdp_header_A_t* mbms_attr = tsk_null;
						
						char* saveptr = tsk_null;
						char* value = tsk_null; 
						uint64_t tmgi_decimal = 0;
						uint64_t tmgi_decimal_local = 0;
						uint32_t tmgi_part = 0;

						if(!(sdp = tsdp_message_parse(content_ptr, content_size)))
							return -1;

						if(!(multicast_media = tsdp_message_find_media_at_index(sdp, "audio", 1)))
							return -1;
						
						if(!(mbms_attr = tsdp_header_M_findA(multicast_media, "mbms-mode")))
							return -1;
						
						value = tsk_strtok_r((char*)mbms_attr->value, " ", &saveptr);
						
						if(!value)
							return -1;
						
						value = tsk_strtok_r(tsk_null, " ", &saveptr);
						if(!value)
							return -1;
						
						sscanf(value, "%llu", &tmgi_decimal);
						tmgi_part = (uint32_t)(tmgi_decimal & 0x00000000FFFFFFFFULL);
						tmgi_decimal_local = ((uint64_t)tnet_ntohl_2(&tmgi_part)) << 32;
						tmgi_part = (uint32_t)(((uint64_t)(tmgi_decimal & 0xFFFFFFFF00000000ULL)) >> 32);
			            tmgi_decimal_local |= tnet_ntohl_2(&tmgi_part);
...
int send_INVITEorUPDATE(tsip_dialog_invite_t *self, tsk_bool_t is_INVITE, tsk_bool_t force_sdp)
{
	...
	tsk_size_t i;
	tsk_bool_t control_accept_contact;
	char* x_Framed_IP_Address_Port_Header=tsk_null;
	tsip_stack_t* sip_stack=tsk_null;
//	tsk_size_t len;
//	char* hdr_value;
	...
	if((request = tsip_dialog_request_new(TSIP_DIALOG(self), is_INVITE ? "INVITE" : "UPDATE"))){
		...
		char* type_Header_Answer="";
		char* value_Header_Answer="";
			...
			if((force_sdp || is_INVITE) || ((self->msession_mgr && self->msession_mgr->state_changed) || (TSIP_DIALOG(self)->state == tsip_initial))){
				if(!TSIP_DIALOG(self)->curr_action || !TSIP_DIALOG(self)->curr_action->payload){
					...
					if((sdp_lo = tmedia_session_mgr_get_lo(self->msession_mgr)) && (sdp = tsdp_message_tostring(sdp_lo))){
							...
							tmedia_session_t* multicast_audio_session = tsk_null;
							//Get from SDP local of ip and port for header "X-Framed-IP-Address-Port"
							sip_stack=TSIP_DIALOG_GET_STACK(self);
							...
							if((TSIP_DIALOG_GET_SS(self)->media.type & tmedia_audio_ptt_group_mcptt) != tmedia_audio_ptt_group_mcptt){//IF isn�t MCPTT GROUP. It is private
								...
								tmedia_content_multipart_t* recipient_list_content = tsk_null;
								tmedia_content_multipart_t* mcptt_info_content = tsk_null;
								tsk_buffer_t* resource_list;
								...
								resource_list = tsip_dialog_invite_create_resource_list(self);
								mcptt_info = tsip_dialog_invite_create_mcpttinfo(self);
								...
								if(body)
								{
								  ...
								  //resource-list
								  recipient_list_content = tmedia_content_multipart_create(TSK_BUFFER_DATA(resource_list), TSK_BUFFER_SIZE(resource_list), "application/resource-lists+xml", "recipient-list");
								  if(recipient_list_content)
									tmedia_content_multipart_body_add_content(body, recipient_list_content);
								  //mcpt-info
							...
							{
								tsk_buffer_t* resource_list = tsip_dialog_create_resource_list_group(TSIP_DIALOG(self), TSIP_DIALOG_GET_SS(self)->pttMCPTT.ptt_group_members);
								char* content_type_hdr  = tsk_null;
								char* body_string = tsk_null;
								tsk_buffer_t* mcptt_info;
							...
							multicast_audio_session = tmedia_session_mgr_find_at_index(self->msession_mgr, tmedia_audio, 1);
							...
							if(multicast_audio_session != tsk_null)
							{
								tmedia_session_mgr_set(self->msession_mgr,
									...
									TMEDIA_SESSION_SET_NULL());
								multicast_audio_session->lo_held = tsk_false;
								multicast_audio_session->ro_held = tsk_false;
						...
						else if (!is_INVITE && (TSIP_DIALOG_GET_SS(self)->media.type & tmedia_audio_ptt_mcptt) == tmedia_audio_ptt_mcptt)//IF it isn�t invite
						{
							...
							audio_session = tmedia_session_mgr_find(self->msession_mgr, tmedia_audio);
							multicast_audio_session = tmedia_session_mgr_find_at_index(self->msession_mgr, tmedia_audio, 1);
							if(multicast_audio_session != tsk_null && audio_session != tsk_null)
							{
								tmedia_session_mgr_set(self->msession_mgr,
									TMEDIA_SESSION_SET_POBJECT(tmedia_mcptt, "multicast_audio_session", multicast_audio_session),
									TMEDIA_SESSION_SET_NULL());
								multicast_audio_session->lo_held = tsk_true;
								if(audio_session->lo_held == tsk_true)
									multicast_audio_session->ro_held = tsk_false;
								else
		...
		if((TSIP_DIALOG_GET_SS(self)->media.type & tmedia_mcptt) == tmedia_mcptt){
			...
			if(x_Framed_IP_Address_Port_Header!=tsk_null){
				tsip_message_add_headers(request,
				TSIP_HEADER_DUMMY_VA_ARGS("X-Framed-IP-Address-Port", x_Framed_IP_Address_Port_Header),
				tsk_null);
			}
		...
		if((TSIP_DIALOG_GET_SS(self)->media.type & tmedia_audio_ptt_mcptt) == tmedia_audio_ptt_mcptt)
		{
			...
			control_accept_contact=tsk_false;
			...
			{		
				...
				if(tsk_strcmp(dummy_hdr->name, "Accept-Contact") == 0)
				{
					...
					TSIP_HEADER_ADD_PARAM(dummy_hdr,"+g.3gpp.icsi-ref=\"urn%3Aurn-7%3A3gpp-service.ims.icsi.mcptt\";require;explicit", tsk_null);
					break;
				}
...
/*				else if(tsk_strcmp(dummy_hdr->name, "User-Agent") == 0)
			...
			if(control_accept_contact==tsk_false){
				tsip_message_add_headers(request,
				TSIP_HEADER_DUMMY_VA_ARGS("Accept-Contact", "*;+g.3gpp.mcptt;require;explicit"),
				tsk_null
				);
				tsip_message_add_headers(request,
				TSIP_HEADER_DUMMY_VA_ARGS("Accept-Contact", "*;+g.3gpp.icsi-ref=\"urn%3Aurn-7%3A3gpp-service.ims.icsi.mcptt\";require;explicit"),
				tsk_null
				);
			}
			...
			/*
			tsip_message_add_headers(request,
				TSIP_HEADER_DUMMY_VA_ARGS("Reject-Contact", "*;sip.automata;sip.actor=msg-taker;require;explicit"),
				tsk_null
			);
			//here it is adding new parameter in request, formate key and value
			tsip_message_add_headers(request,
				TSIP_HEADER_DUMMY_VA_ARGS("Reject-Contact", "*;sip.automata;sip.actor=principal;sip.description=\"poc recording device\";require;explicit"),
				tsk_null
			);
			...
			tsip_message_add_headers(request,
				TSIP_HEADER_DUMMY_VA_ARGS("Priv-Answer-Mode", "Auto"),
				tsk_null
			);
			*/
		}
	...
	if(x_Framed_IP_Address_Port_Header!=tsk_null){
		TSK_FREE(x_Framed_IP_Address_Port_Header);
	}
...
int send_ACK(tsip_dialog_invite_t *self, const tsip_response_t* r2xxINVITE)
{
	...
	dialog=TSIP_DIALOG(self);
	if((request = tsip_dialog_request_new(dialog, "ACK"))){
		request->Contact=tsk_null;
		...
		*/
		if(self->is_client && (self->last_oInvite && !TSIP_MESSAGE_HAS_CONTENT(self->last_oInvite))){
			...
			if(!self->msession_mgr->started && (self->msession_mgr->sdp.lo && self->msession_mgr->sdp.ro)){
				/* Set MSRP Callback */
				if((self->msession_mgr->type & tmedia_msrp) == tmedia_msrp){
					tmedia_session_mgr_set_msrp_cb(self->msession_mgr, 
						TSIP_DIALOG_GET_SS(self)->userdata, TSIP_DIALOG_GET_SS(self)->media.msrp.callback);
				}
				// starts session manager
				ret = tsip_dialog_invite_msession_start(self);
			}
		...
		if(TSIP_REQUEST_IS_INVITE(request) || TSIP_REQUEST_IS_UPDATE(request)){
			...
			if(self->msession_mgr && force_sdp){
				...
				if(((TSIP_DIALOG_GET_SS(self)->media.type & tmedia_audio_ptt_mcptt) == tmedia_audio_ptt_mcptt) && ((sdp_lo = tmedia_session_mgr_get_lo(self->msession_mgr)) && (sdp = tsdp_message_tostring(sdp_lo)))){
					...
					ret = tsip_message_add_content(response, content_type_hdr, body_string, tsk_strlen(body_string));							
...
static tsk_buffer_t* tsip_dialog_invite_create_resource_list(const tsip_dialog_invite_t* self)
{
	...
	char* ret = tsk_null;
	int numMembers=0;
	tsip_ssession_t* ptsip_ssession_t;
	int con;
/*
#if HAVE_LIBXML2
	ret = tsk_null;
	...
	const char* headout  = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n"
						"<resource-lists>\r\n"
						"  <list>\r\n";
	const char* tail =  "  </list>\r\n"
						"</resource-lists>\r\n";
	ptsip_ssession_t=TSIP_SSESSION(self);
	/*if(ptsip_ssession_t->stack->pttMCPTT.mcptt_namespace==tsk_true){*/
...
static tsk_buffer_t* tsip_dialog_invite_create_mcpttinfo(const tsip_dialog_invite_t* self)
{
	tsip_stack_t* ptsip_stack_t=TSIP_DIALOG_GET_STACK(self);
	tsk_buffer_t* output = tsk_buffer_create_null();
	...
	//create xml with libxml
	/* 
     * Creates a new document, a node and set it as a root node
     */
	session_sip=TSIP_DIALOG_GET_SS(self);
    doc = xmlNewDoc(BAD_CAST "1.0");
    root_node = xmlNewNode(NULL, BAD_CAST "mcpttinfo");
    ns = xmlNewNs(root_node, BAD_CAST"urn:3gpp:ns:mcpttInfo:1.0",NULL);
    xsi = xmlNewNs(root_node, BAD_CAST"http://www.w3.org/2001/XMLSchema-instance", BAD_CAST "xsi");
	...
	const char* headout  = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n"
						"<mcpttinfo>\r\n"
						"<mcptt-Params>\r\n";
	const char* tabInitSessionType =  "<session-type>";//String
	...
	const char* tabInitRequestUriOld =  "<mcptt-request-uri>";
	...
	if((TSIP_DIALOG_GET_SS(self)->media.type & tmedia_audio_ptt_group_mcptt) != tmedia_audio_ptt_group_mcptt){
		//<session-type>
		tsk_buffer_append_2(output, "%s", tabInitSessionType);
		tsk_buffer_append_2(output, "%s", "private");
		tsk_buffer_append_2(output, "%s", tabEndSessionType);
		...
		*/
		...
		//<session-type>
		tsk_buffer_append_2(output, "%s", tabInitSessionType);
		tsk_buffer_append_2(output, "%s", "prearranged");
		tsk_buffer_append_2(output, "%s", tabEndSessionType);
		//<mcptt-request-uri>

			tsk_buffer_append_2(output, "%s", tabInitRequestUri);
			tsk_buffer_append_2(output, "%s", tabInitMcpttURI);
		tsk_buffer_append_2(output, "%s", TSIP_DIALOG_GET_SS(self)->pttMCPTT.ptt_group_uri);
...
static tsk_buffer_t* tsip_dialog_200ok_create_mcpttinfo(const tsip_dialog_invite_t* self)
{
	...
	const char* tabEndSessionType =  "</session-type>\r\n";
	const char* tabInitMCPTTID =  "<mcptt-called-party-id type=\"Normal\">";
	const char* tabInitMCPTTID_old =  "<mcptt-called-party-id>";
	const char* tabEndMCPTTID =  "</mcptt-called-party-id>\r\n";
