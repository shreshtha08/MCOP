...
/*#define TSIP_DIALOG_REGISTER_AUTHENTICATION_SIGNAL(self, type, code, phrase, message)	\
	tsip_register_authentication_event_signal(type, TSIP_DIALOG(self)->ss, code, phrase, message)*/
...
int tsip_dialog_register_send_REGISTER(tsip_dialog_register_t *self, tsk_bool_t initial)
{
		...
		*/
		...
		if(TSIP_DIALOG(self)->curr_action){
			...
			if(TSIP_DIALOG(self)->curr_action->payload){
				tsip_message_add_content(request, "application/vnd.3gpp.mcptt-info+xml", TSK_BUFFER_DATA(TSIP_DIALOG(self)->curr_action->payload), TSK_BUFFER_SIZE(TSIP_DIALOG(self)->curr_action->payload));
			}	
	...
	TSIP_DIALOG_REGISTER_AUTHENTICATION_SIGNAL(self, tsip_i_register_authentication, 
